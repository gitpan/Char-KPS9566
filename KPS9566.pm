...
my $qq_substr  = qr{(?: KPS9566::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_ord;         # ord()   to ord()   or KPS9566::ord()
my $function_ord_;        # ord     to ord     or KPS9566::ord_
my $function_reverse;     # reverse to reverse or KPS9566::reverse
my $function_getc;        # getc    to getc    or KPS9566::getc
...
sub import {
    ...
    if (Ekps9566::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            ...
            my $e_mtime   = (Ekps9566::stat("$filename.e"))[9];
            my $mtime     = (Ekps9566::stat($filename))[9];
            my $__mtime__ = (Ekps9566::stat(__FILE__))[9];
...
sub KPS9566::escape_script {
    ...
    if (/^ use Ekps9566(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $function_getc    = 'getc';
        if (s/^ \s* use \s+ KPS9566 \s* ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'KPS9566::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'KPS9566::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'KPS9566::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'KPS9566::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Ekps9566
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Ekps9566::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Ekps9566::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Ekps9566::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $slash = 'm//'; return "Ekps9566::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Ekps9566::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Ekps9566::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Ekps9566::$1(" . e_q ('q', $2,$4,$3) . ")"; }
...
# functions of package Ekps9566
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Ekps9566::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Ekps9566::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Ekps9566::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $e_string .= "Ekps9566::$1(" . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Ekps9566::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Ekps9566::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Ekps9566::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $KPS9566::re_t=%s; local $KPS9566::re_a=''; while($KPS9566::re_t =~ %s){%s local $^W=0; local $KPS9566::re_r=%s; %s$KPS9566::re_t="$KPS9566::re_a${1}$KPS9566::re_r$'"; pos($KPS9566::re_t)=length "$KPS9566::re_a${1}$KPS9566::re_r"; $KPS9566::re_a=substr($KPS9566::re_t,0,pos($KPS9566::re_t)); } return $KPS9566::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$KPS9566::re_r=eval $KPS9566::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $KPS9566::re_t=%s; while($KPS9566::re_t =~ %s){%s local $^W=0; local $KPS9566::re_r=%s; %s$KPS9566::re_t="$`$KPS9566::re_r$'"; pos($KPS9566::re_t)=length "$`$KPS9566::re_r"; } return $KPS9566::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $KPS9566::re_r=%s; %s"%s$KPS9566::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $KPS9566::re_n=0; local $KPS9566::re_a=''; while(%s =~ %s){%s local $^W=0; local $KPS9566::re_r=%s; %s%s="$KPS9566::re_a${1}$KPS9566::re_r$'"; pos(%s)=length "$KPS9566::re_a${1}$KPS9566::re_r"; $KPS9566::re_a=substr(%s,0,pos(%s)); $KPS9566::re_n++} return %s$KPS9566::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $KPS9566::re_n=0; while(%s =~ %s){%s local $^W=0; local $KPS9566::re_r=%s; %s%s="$`$KPS9566::re_r$'"; pos(%s)=length "$`$KPS9566::re_r"; $KPS9566::re_n++} return %s$KPS9566::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $KPS9566::re_r=%s; %s%s="%s$KPS9566::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $KPS9566::re_r=%s; %s%s="%s$KPS9566::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$KPS9566::re_r=eval $KPS9566::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Ekps9566.pm
works well.
...
=head1 Escaping Multiple-Octet Code (KPS9566.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (KPS9566.pm provides)

KPS9566.pm applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (KPS9566.pm provides)

KPS9566.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (KPS9566.pm provides)

KPS9566.pm clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Ekps9566::ignorecase()' (KPS9566.pm provides)

KPS9566.pm applies calling 'Ekps9566::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   eval{ Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Ekps9566::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  eval{ Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Definitions in Ekps9566.pm.
  ...
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Ekps9566::anchor}         qr{\G(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE])*?}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (KPS9566.pm provides)

KPS9566.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (KPS9566.pm and Ekps9566.pm provide)
...
=head1 Escaping File Test Operators (KPS9566.pm and Ekps9566.pm provide)
...
=item * Returns Next Character
  ...
  If you import getc "use KPS9566 qw(getc);", getc of your script will be rewritten in
  KPS9566::getc. KPS9566::getc is not compatible with getc of JPerl.
...
=item * Length by KPS9566 Character
  ...
  the scalar value $string. If $string is omitted, it returns the KPS9566::length of
  $_.
...
=item * Substr by KPS9566 Character
  ...
  If Perl version 5.14 or later, you can use the KPS9566::substr() function as an
...
=item * Rindex by KPS9566 Character
  ...
  This function works just like KPS9566::index except that it returns the character
...
=item * Filename Globbing
  ...
  This operator is implemented via the Ekps9566::glob() function. See Ekps9566::glob of
  Ekps9566.pm for details.
...
=head1 Un-Escaping bytes::* Functions (KPS9566.pm provides)

KPS9566.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Ekps9566.pm provides)

Ekps9566.pm does "BEGIN { unshift @INC, '/Perl/site/lib/KPS9566' }" at head.
Store the standard module modified for KPS9566 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/KPS9566 and change
'use utf8;' to 'use KPS9566;' in its. You need help yourself for now.
...
KPS9566::substr differs from CORE::substr, and cannot be used as a lvalue.
