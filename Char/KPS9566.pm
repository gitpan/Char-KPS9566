...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Ekps9566::function_ord     = 'Char::KPS9566::ord'     if /\A ord \z/oxms;
                    $Char::Ekps9566::function_ord_    = 'Char::KPS9566::ord_'    if /\A ord \z/oxms;
                    $Char::Ekps9566::function_reverse = 'Char::KPS9566::reverse' if /\A reverse \z/oxms;
                    $Char::Ekps9566::function_getc    = 'Char::KPS9566::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::KPS9566::escape();
...
=head1 Escaping Multiple-Octet Code (Char::KPS9566 software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::KPS9566 software provides)

Char::KPS9566 software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (Char::KPS9566 software provides)

Char::KPS9566 software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::KPS9566 software provides)

Char::KPS9566 software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Ekps9566::ignorecase()' (Char::KPS9566 software provides)

Char::KPS9566 software applies calling 'Char::Ekps9566::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Ekps9566::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::KPS9566 software provides)

Char::KPS9566 software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::KPS9566 software provide)
...
=head1 Escaping File Test Operators (Char::KPS9566 software provide)
...
=item * Eval KPS9566 Script
  ...
  $eval = Char::KPS9566::eval { block };
  $eval = Char::KPS9566::eval $expr;
  $eval = Char::KPS9566::eval;

  The Char::KPS9566::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::KPS9566::eval { block }
  and Char::KPS9566::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::KPS9566::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::KPS9566::eval, the value returned from an Char::KPS9566::eval is
  ...
  Char::KPS9566::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::KPS9566::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::KPS9566::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::KPS9566::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::KPS9566::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::KPS9566::eval cannot trap syntax errors.

  The Char::KPS9566::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::KPS9566::eval is complete, as do any subroutine or format definitions.
  The code of the Char::KPS9566::eval is treated as a block, so any locally scoped
  variables declared within the Char::KPS9566::eval last only until the Char::KPS9566::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::KPS9566::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::KPS9566::eval { block }    eval { block }
  Char::KPS9566::eval $expr        eval Char::KPS9566::escape $expr
  Char::KPS9566::eval              eval Char::KPS9566::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::KPS9566::eval does not exist. If it exists,
  you will troubled, when Char::KPS9566::eval has a parameter that is single quoted
  string included my variables. Char::KPS9566::escape is a subroutine that makes Perl
  ...
  Since Char::KPS9566::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::KPS9566::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::KPS9566::eval $expr and Char::KPS9566::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::KPS9566::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::KPS9566::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::KPS9566::eval '$answer =';   # sets $@
  ...
  Using the Char::KPS9566::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::KPS9566::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::KPS9566::eval { die "foo lives here" };
  ...
  With an Char::KPS9566::eval, you should be especially careful to remember what's being
  looked at when:

      Char::KPS9566::eval $x;        # CASE 1
      Char::KPS9566::eval "$x";      # CASE 2

      Char::KPS9566::eval '$x';      # CASE 3
      Char::KPS9566::eval { $x };    # CASE 4

      Char::KPS9566::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::KPS9566::eval { test_repugnancy() };
  ...
  The block of Char::KPS9566::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::KPS9566 software provides)

Char::KPS9566 software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::KPS9566 software and
