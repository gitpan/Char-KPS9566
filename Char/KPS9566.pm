package Char::KPS9566;
######################################################################
#
# Char::KPS9566 - Source code filter to escape KPS9566 script
#
# http://search.cpan.org/dist/Char-Char::KPS9566/
#
...
BEGIN { CORE::require Char::Ekps9566; }
...
sub unimport {}
sub Char::KPS9566::escape_script;
...
my $function_ord;         # ord()   to ord()   or Char::KPS9566::ord()
my $function_ord_;        # ord     to ord     or Char::KPS9566::ord_
my $function_reverse;     # reverse to reverse or Char::KPS9566::reverse
my $function_getc;        # getc    to getc    or Char::KPS9566::getc
...
END
    ...
    print Char::KPS9566::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Char::Ekps9566::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            Char::Ekps9566::unlink "$filename.e";
        }
        elsif (Char::Ekps9566::z("$filename.e")) {
            Char::Ekps9566::unlink "$filename.e";
        }
        else {
            my $e_mtime   = (Char::Ekps9566::stat("$filename.e"))[9];
            my $mtime     = (Char::Ekps9566::stat($filename))[9];
            my $__mtime__ = (Char::Ekps9566::stat(__FILE__))[9];
    ...
    if (not Char::Ekps9566::e("$filename.e")) {
        my $fh = gensym();

        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
        }
        else {
            Char::Ekps9566::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::KPS9566::escape_script($filename);
        print {$fh} $e_script;

        my $mode = (Char::Ekps9566::stat($filename))[2] & 0777;
    ...
    my $fh = gensym();
    Char::Ekps9566::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::KPS9566::escape_script {
    ...
    # read KPS9566 script
    my $fh = gensym();
    Char::Ekps9566::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ekps9566(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ekps9566 %s;\n", $Char::KPS9566::VERSION); # require run-time routines version

        # use Char::KPS9566 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::KPS9566 \s* ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::KPS9566::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/KPS9566.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/KPS9566.pm $version, but @{[__FILE__]} is version $Char::KPS9566::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ekps9566.pm $version, but \\\$Char::Ekps9566::VERSION is \$Char::Ekps9566::VERSION" if \$Char::Ekps9566::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::KPS9566::VERSION) {
                    die "Script $0 required Char/KPS9566.pm $version, but @{[__FILE__]} is only version $Char::KPS9566::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::KPS9566::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::KPS9566::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::KPS9566::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::KPS9566::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Ekps9566
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Char::Ekps9566::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Char::Ekps9566::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Char::Ekps9566::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Char::Ekps9566::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Char::Ekps9566::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Char::Ekps9566::$1(" . e_q ('q', $2,$4,$3) . ")"; }
...
# functions of package Char::Ekps9566
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Char::Ekps9566::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Char::Ekps9566::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Char::Ekps9566::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Char::Ekps9566::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Char::Ekps9566::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Char::Ekps9566::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::KPS9566::re_t=%s; local $Char::KPS9566::re_a=''; while($Char::KPS9566::re_t =~ %s){%s local $^W=0; local $Char::KPS9566::re_r=%s; %s$Char::KPS9566::re_t="$Char::KPS9566::re_a${1}$Char::KPS9566::re_r$'"; pos($Char::KPS9566::re_t)=length "$Char::KPS9566::re_a${1}$Char::KPS9566::re_r"; $Char::KPS9566::re_a=substr($Char::KPS9566::re_t,0,pos($Char::KPS9566::re_t)); } return $Char::KPS9566::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::KPS9566::re_r=eval $Char::KPS9566::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::KPS9566::re_t=%s; while($Char::KPS9566::re_t =~ %s){%s local $^W=0; local $Char::KPS9566::re_r=%s; %s$Char::KPS9566::re_t="$`$Char::KPS9566::re_r$'"; pos($Char::KPS9566::re_t)=length "$`$Char::KPS9566::re_r"; } return $Char::KPS9566::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::KPS9566::re_r=%s; %s"%s$Char::KPS9566::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::KPS9566::re_n=0; local $Char::KPS9566::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::KPS9566::re_r=%s; %s%s="$Char::KPS9566::re_a${1}$Char::KPS9566::re_r$'"; pos(%s)=length "$Char::KPS9566::re_a${1}$Char::KPS9566::re_r"; $Char::KPS9566::re_a=substr(%s,0,pos(%s)); $Char::KPS9566::re_n++} return %s$Char::KPS9566::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::KPS9566::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::KPS9566::re_r=%s; %s%s="$`$Char::KPS9566::re_r$'"; pos(%s)=length "$`$Char::KPS9566::re_r"; $Char::KPS9566::re_n++} return %s$Char::KPS9566::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::KPS9566::re_r=%s; %s%s="%s$Char::KPS9566::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::KPS9566::re_r=%s; %s%s="%s$Char::KPS9566::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::KPS9566::re_r=eval $Char::KPS9566::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/KPS9566.pm and Char/Ekps9566.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::KPS9566;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::KPS9566;
  use Char::KPS9566 ver.sion;             --- require minimum version
  use Char::KPS9566 ver.sion.0;           --- expects version (match or die)
  use Char::KPS9566 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::KPS9566 ver.sion qw(ord reverse getc);
  use Char::KPS9566 ver.sion.0 qw(ord reverse getc);

  # "no Char::KPS9566;" not supported

  or

  $ perl Char/KPS9566.pm KPS9566_script.pl > Escaped_script.pl.e
...
Char::KPS9566 software is "middleware" between perl interpreter and your Perl script
written in KPS9566.
...
By "use Char::KPS9566;", it automatically interpret your script as KPS9566. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/KPS9566.pm, Char/Ekps9566.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ekps9566.pm
works well.
...
=head1 Software Composition
   ...
   Char/KPS9566.pm               --- source code filter to escape KPS9566
   Char/Ekps9566.pm              --- run-time routines for Char/KPS9566.pm
...
You need write 'use Char::KPS9566;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::KPS9566;
  ---------------------------------

=head1 Escaping Multiple-Octet Code (Char/KPS9566.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/KPS9566.pm provides)

Char/KPS9566.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Ekps9566::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (Char/KPS9566.pm provides)

Char/KPS9566.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char/KPS9566.pm provides)

Char/KPS9566.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Ekps9566::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Ekps9566::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Ekps9566::ignorecase()' (Char/KPS9566.pm provides)

Char/KPS9566.pm applies calling 'Char::Ekps9566::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ekps9566::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Ekps9566::matched/
  m/regexp/               m/ditto$Char::Ekps9566::matched/
  ?regexp?                m?ditto$Char::Ekps9566::matched?
  m?regexp?               m?ditto$Char::Ekps9566::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Ekps9566::matched/) ?
  s/regexp/replacement/   eval{ Char::Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Ekps9566::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Ekps9566::split(qr/regexp/)
  split(m/regexp/)        Char::Ekps9566::split(qr/regexp/)
  split(qr/regexp/)       Char::Ekps9566::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Ekps9566::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Ekps9566::matched/
  m/regexp/b              m/(?:regexp)$Char::Ekps9566::matched/
  ?regexp?b               m?regexp$Char::Ekps9566::matched?
  m?regexp?b              m?regexp$Char::Ekps9566::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Ekps9566::matched/) ?
  s/regexp/replacement/b  eval{ Char::Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Ekps9566::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ekps9566.pm provides)
...
The character classes are redefined as follows to backward compatibility.
  ...
  ---------------------------------------------------------------
   .            ${Char::Ekps9566::dot}
                ${Char::Ekps9566::dot_s}    (/s modifier)
  \d            [0-9]
  \s            \s
  \w            [0-9A-Z_a-z]
  \D            ${Char::Ekps9566::eD}
...
Definitions in Char/Ekps9566.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Char::Ekps9566::anchor}         qr{\G(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE])*?}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/KPS9566.pm provides)

Char/KPS9566.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/KPS9566.pm and Char/Ekps9566.pm provide)

Insert 'Char::Ekps9566::' at head of function name. Char/Ekps9566.pm provides your script Char::Ekps9566::*
functions.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Ekps9566::split     Character
  tr///       Char::Ekps9566::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Ekps9566::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Ekps9566::chop      Character
  index       Char::Ekps9566::index     Character
  rindex      Char::Ekps9566::rindex    Character
  lc          Char::Ekps9566::lc        Character
  lcfirst     Char::Ekps9566::lcfirst   Character
  uc          Char::Ekps9566::uc        Character
  ucfirst     Char::Ekps9566::ucfirst   Character
  fc          Char::Ekps9566::fc        Character
  chr         Char::Ekps9566::chr       Character
  glob        Char::Ekps9566::glob      Character
  lstat       Char::Ekps9566::lstat     Character
  opendir     Char::Ekps9566::opendir   Character
  stat        Char::Ekps9566::stat      Character
  unlink      Char::Ekps9566::unlink    Character
  chdir       Char::Ekps9566::chdir     Character
  do          Char::Ekps9566::do        Character
  require     Char::Ekps9566::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Char::Ekps9566::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Char::Ekps9566::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Char::Ekps9566::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Char::Ekps9566::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Char::Ekps9566::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Char::Ekps9566::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Char/KPS9566.pm and Char/Ekps9566.pm provide)

Insert 'Char::Ekps9566::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Char::Ekps9566::r   File or directory is readable by this (effective) user or group
  -w       Char::Ekps9566::w   File or directory is writable by this (effective) user or group
  -e       Char::Ekps9566::e   File or directory name exists
  -x       Char::Ekps9566::x   File or directory is executable by this (effective) user or group
  -z       Char::Ekps9566::z   File exists and has zero size (always false for directories)
  -f       Char::Ekps9566::f   Entry is a plain file
  -d       Char::Ekps9566::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Ekps9566::T   File looks like a "text" file
  -B       Char::Ekps9566::B   File looks like a "binary" file
  -M       Char::Ekps9566::M   Modification age (measured in days)
  -A       Char::Ekps9566::A   Access age (measured in days)
  -C       Char::Ekps9566::C   Inode-modification age (measured in days)
  -s       Char::Ekps9566::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Ekps9566::R   File or directory is readable by this real user or group
  -W       Char::Ekps9566::W   File or directory is writable by this real user or group
  -X       Char::Ekps9566::X   File or directory is executable by this real user or group
  -l       Char::Ekps9566::l   Entry is a symbolic link
  -S       Char::Ekps9566::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Ekps9566::o   File or directory is owned by this (effective) user
  -O       Char::Ekps9566::O   File or directory is owned by this real user
  -p       Char::Ekps9566::p   Entry is a named pipe (a "fifo")
  -b       Char::Ekps9566::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Ekps9566::c   Entry is a character-special file (like an I/O device)
  -u       Char::Ekps9566::u   File or directory is setuid
  -g       Char::Ekps9566::g   File or directory is setgid
  -k       Char::Ekps9566::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Char::KPS9566::' at head of function name when you want character-
oriented function. See 'Character-Oriented Functions'.

  --------------------------------------------------------
  Function   Character-Oriented   Description
  --------------------------------------------------------
  ord        Char::KPS9566::ord
  reverse    Char::KPS9566::reverse
  getc       Char::KPS9566::getc
  length     Char::KPS9566::length
  substr     Char::KPS9566::substr
  index      Char::KPS9566::index          See 'About Indexes'
  rindex     Char::KPS9566::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Ekps9566::index)
  Char::KPS9566::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Ekps9566::rindex)
  Char::KPS9566::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = Char::KPS9566::ord($string);

  This function returns the numeric value (ASCII or KPS9566 character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use Char::KPS9566 qw(ord);", ord of your script will be rewritten in
  Char::KPS9566::ord. Char::KPS9566::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::KPS9566::reverse(@list);
  $reverse = Char::KPS9566::reverse(@list);
  ...
  If you import reverse "use Char::KPS9566 qw(reverse);", reverse of your script will be
  rewritten in Char::KPS9566::reverse. Char::KPS9566::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::KPS9566::getc(FILEHANDLE);
  $getc = Char::KPS9566::getc($filehandle);
  $getc = Char::KPS9566::getc;
  ...
  If you import getc "use Char::KPS9566 qw(getc);", getc of your script will be rewritten in
  Char::KPS9566::getc. Char::KPS9566::getc is not compatible with getc of JPerl.

=item * Length by KPS9566 Character

  $length = Char::KPS9566::length($string);
  $length = Char::KPS9566::length();

  This function returns the length in characters (programmer-visible characters) of
  the scalar value $string. If $string is omitted, it returns the Char::KPS9566::length of
  $_.

  Do not try to use Char::KPS9566::length to find the size of an array or hash. Use scalar
...
=item * Substr by KPS9566 Character
  ...
  $substr = Char::KPS9566::substr($string,$offset,$length,$replacement);
  $substr = Char::KPS9566::substr($string,$offset,$length);
  $substr = Char::KPS9566::substr($string,$offset);
  ...
  my $color  = Char::KPS9566::substr $s, 4, 5;      # black
  my $middle = Char::KPS9566::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::KPS9566::substr $s, 14;        # climbed the green tree
  my $tail   = Char::KPS9566::substr $s, -4;        # tree
  my $z      = Char::KPS9566::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::KPS9566::substr() function as an
  ...
  end of the string, Char::KPS9566::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::KPS9566::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::KPS9566::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::KPS9566::substr $name, 7;      # returns undef, with warning
  Char::KPS9566::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::KPS9566::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::KPS9566::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::KPS9566::substr() acts
...
=item * Index by KPS9566 Character
  ...
  $index = Char::KPS9566::index($string,$substring,$offset);
  $index = Char::KPS9566::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::KPS9566::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by KPS9566 Character
  ...
  $rindex = Char::KPS9566::rindex($string,$substring,$offset);
  $rindex = Char::KPS9566::rindex($string,$substring);

  This function works just like Char::KPS9566::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::KPS9566::index).
  ...
  $pos = Char::KPS9566::length($string);
  while (($pos = Char::KPS9566::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Ekps9566::glob() function. See Char::Ekps9566::glob of
  Char/Ekps9566.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::KPS9566 qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::KPS9566 qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Functions (Char/KPS9566.pm provides)

Char/KPS9566.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Ekps9566.pm provides)

Char/Ekps9566.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::KPS9566' }" at head.
Store the standard module modified for Char::KPS9566 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::KPS9566 and change
'use utf8;' to 'use Char::KPS9566;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ekps9566.pm provides these
  functions.
...
=item * Char::KPS9566::substr as Lvalue

Char::KPS9566::substr differs from CORE::substr, and cannot be used as a lvalue.
To change part of a string, you can use the optional fourth argument which is the
replacement string.

Char::KPS9566::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Ekps9566::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Ekps9566::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Ekps9566::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Ekps9566::MATCH()       $1
  $MATCH          Char::Ekps9566::MATCH()       $1
  ${^MATCH}       Char::Ekps9566::MATCH()       $1
  $'              Char::Ekps9566::POSTMATCH()   $'
  $POSTMATCH      Char::Ekps9566::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Ekps9566::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Ekps9566::*(),
Char::Ekps9566::lstat(), and Char::Ekps9566::stat() on Microsoft Windows open the file for the path
...
Char::KPS9566 software remains one language and one interpreter.
